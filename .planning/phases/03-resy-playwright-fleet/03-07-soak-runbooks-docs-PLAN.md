...
must_haves:
  truths:
    ...
    - "A run that collected zero Playwright processes, or fewer samples than the duration and interval imply, exits 2 (preconditions not met) rather than 0 — an unfalsifiable green is not a pass. Exit codes mirror `scripts/check_poll_success.py`: 0 pass, 1 gate failed, 2 setup/insufficient data."

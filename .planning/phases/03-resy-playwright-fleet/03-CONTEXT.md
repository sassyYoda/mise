...
- **D-61:** Sessions are pre-authenticated from env `RESY_ACCOUNTS_JSON` (JSON array of `{"email": "...", "cookies": [{"name","value","domain","path", ...}]}`; the Phase 1 runbook shape), injected via `context.add_cookies(...)`; cookies exist only in context memory (never DB, never logs — `shared/telemetry.py` redaction covers `cookie`/`auth_token`/`x-resy-auth-token`). Accounts are assigned to contexts round-robin; with fewer accounts than contexts the accounts repeat. If `RESY_ACCOUNTS_JSON` is empty/unset the fleet starts in **anonymous mode** (contexts without cookies) and logs a `resy_anonymous_mode` warning once — the pool still runs so the stub-backed tests and the soak script work without secrets.

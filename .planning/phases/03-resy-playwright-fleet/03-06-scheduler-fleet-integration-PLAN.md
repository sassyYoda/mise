...
must_haves:
  truths:
    ...
    - "A successful Resy poll DELetes `backoff:{source}:{rid}`, so the next interval returns to the tier cadence; consecutive failures escalate `min(interval * 2 ** n, 1800)` and the key's TTL is twice its value so a stalled job self-heals."

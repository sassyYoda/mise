...
### Amendments after research (05-RESEARCH.md §Blocking Corrections BC-1..BC-3 + §Open Questions — accepted 2026-09-05)
- **D-104a (BC-1):** `httpx.ASGITransport` buffers the whole response and never runs lifespan, so SSE tests (first-event latency, heartbeat, `Last-Event-ID` replay, disconnect cleanup) and any test that needs `app.state` from lifespan run against an in-process `uvicorn.Server` on a random free port (Phase 3 D-71 recipe; measured ~1 ms publish→receipt). Plain request/response routes may still use `ASGITransport` with the app's dependencies overridden; document which tier each test uses in the test module docstring.
- **D-96a (BC-2):** Client IP for rate limiting = the socket peer when `TRUST_PROXY_HEADERS=false`; when `true`, the **second-to-last** entry of `X-Forwarded-For` (`PROXY_HOPS`, default 1 — Google's LB appends `<client>,<lb>`), never the first entry (spoofable) and never uvicorn's default `ProxyHeadersMiddleware` resolution (which collapses everyone onto the LB address). Unit tests cover spoofed-first-hop, single-hop, and no-header cases.
- **D-97a (BC-3):** Starlette's `ServerErrorMiddleware` re-raises after a custom 500 handler, leaking tracebacks (with bound SQL parameters) into the log stream. Unhandled exceptions are therefore caught by our own outermost pure-ASGI middleware (`services/api/middleware.py :: ErrorBoundary`, installed as the last `add_middleware` so it is innermost to Starlette's error middleware but outermost to everything of ours): it logs `safe_error(exc)` + `request_id`, returns the JSON body, and never re-raises. The request-logging middleware is pure ASGI too (records duration/status at the final `http.response.body` of streams). uvicorn runs with `log_config=None` and its `uvicorn.error`/`uvicorn.access` loggers are routed through structlog (`shared/telemetry.py`).
- **D-101a:** `Instrumentator(should_exclude_streaming_duration=True, excluded_handlers=["/api/metrics", "/healthz", "/readyz"])` so a long-lived SSE connection never lands in `http_request_duration_seconds`.
- **D-93a:** "today" for `date_from >= today` is evaluated in `America/New_York`; `_SERVICE_TZ` moves from `services/state_machine/persistence.py` to `shared/servicetime.py` (re-exported from its old name). `Restaurant.party_sizes` is generic `ARRAY` — membership checks use `func.array_position(...)` / `any_()`; the ORM `Restaurant.slug` `unique=True` declaration must already have been replaced by the `(slug, source)` constraint in Phase 3 plan 03-03 (`shared/db.py`) — Wave 0 asserts migrations 0009 and 0010, `shared/tokens.py`, `shared/crypto.py`, `shared/metrics.py`, `services/api/app.py` and the `watch:count`/`tier:override` helpers exist before any API task runs.
- **D-93b (OQ duplicate watches):** `POST /watches` is idempotent per user: migration 0012 (`down_revision="0011"` — Phase 6 owns 0011; if Phase 6 has not created it yet when this phase executes, use the next free number and note it) adds a partial unique index on `watchlist_entries (user_id, restaurant_id, party_size, date_from, date_to, coalesce(time_window_from,'00:00'), coalesce(time_window_to,'23:59'), coalesce(days_of_week,''), coalesce(seat_type_filter,''))` `WHERE status IN ('active','paused')`; a duplicate returns `200` with the existing watch (and a fresh token) instead of `201`.
- **D-95a:** `recount(restaurant_slug)` runs inline in the request and is non-fatal (logged on Redis failure; the poller tolerates a stale count). Tokens never appear in logs: the request logger renders the path template, and query strings are dropped.
- **D-104b:** The OpenAPI snapshot pins the sorted route list (`method path`) plus response model names, not the full document.
- **D-95b:** `shared/redis_keys.py :: watch_count_field(source, restaurant_id)` / `tier_override_field(...)` already exist (Phase 3 plan 03-02); their `restaurant_id` argument is the platform id per D-52 (the poller's job descriptor `{source}:{platform_id}`), so the API passes `int(restaurant.platform_id)` for each source row of the slug — never `restaurants.id`. A unit test pins the produced field for a Resy and an OpenTable row. Migration numbering (D-93b) is resolved at execution time to the next free revision.
- **D-98a:** The feed consumer reuses `_assert_topics_exist` (a missing topic kills a `group_id=None` consumer at start); `commit()` is never called on it.

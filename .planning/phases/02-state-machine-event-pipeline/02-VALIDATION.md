...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 02-01-01 | 01 | 1 | STATE-02, STATE-04 | — | N/A | unit | `uv run pytest tests/unit/test_tracer_raw_to_event.py -x -q` | ❌ W0 (task creates it) | ⬜ pending |
| 02-01-02 | 01 | 1 | STATE-02 | — | N/A | unit | `uv run pytest tests/unit/test_engine_transitions.py tests/unit/test_coverage_bounding.py tests/unit/test_engine_tristate_unknown.py -x -q` | ❌ W0 (task creates it) | ⬜ pending |
| 02-01-03 | 01 | 1 | STATE-04, STATE-06 | T-02-02 | Malformed third-party JSON collapses to `ParseError`, never an unhandled exception that stalls the partition | unit | `uv run pytest tests/unit -q` | ❌ W0 (task creates it) | ⬜ pending |
| 02-02-01 | 02 | 1 | STATE-03 | T-02-01 | Expedite can only pull a queued poll forward to `now+8000`; `XX` cannot resurrect an in-flight job; flag self-expires in 120 s | integration | `uv run pytest tests/integration/test_expedite_lua.py tests/integration/test_poller_expedite_release.py -q -p no:cacheprovider` | ❌ W0 (task creates it) | ⬜ pending |
| 02-02-02 | 02 | 1 | STATE-01, STATE-04 | — | Zero two-command idempotency claims in the source tree (ROADMAP SC3 mechanical half) | unit | `uv run pytest tests/unit -q` | ❌ W0 (task creates it) | ⬜ pending |
| 02-02-03 | 02 | 1 | STATE-05 | — | N/A | integration | `uv run pytest tests/integration/test_migration_0008.py -q -p no:cacheprovider` | ❌ W0 (task creates it) | ⬜ pending |
| 02-03-01 | 03 | 2 | STATE-01, STATE-02, STATE-03, STATE-04, STATE-05 | T-02-02, T-02-03 | Poison message logs and commits without stalling; no `booking_token` or `raw_response` at INFO | integration | `uv run pytest tests/integration/test_state_machine_e2e.py -q -p no:cacheprovider` | ❌ W0 (task creates it) | ⬜ pending |
...
| 02-03-03 | 03 | 2 | STATE-01, STATE-03, STATE-04 | — | No inline async sleep anywhere in the service (STATE-03 gate) | unit + integration | `uv run pytest tests/unit -q && uv run pytest tests/integration/test_redis_state_store.py -q -p no:cacheprovider` | ❌ W0 (task creates it) | ⬜ pending |
| 02-04-01 | 04 | 2 | STATE-06 | T-02-05 | `--output` refused outside the repository root unless absolute | unit | `uv run pytest tests/unit/test_replay_determinism.py -q` | ❌ W0 (task creates it) | ⬜ pending |
| 02-04-02 | 04 | 2 | STATE-06 | — | N/A | unit | `uv run pytest tests/unit/test_replay_zero_false_events.py -q` | ❌ W0 (task creates it) | ⬜ pending |
| 02-04-03 | 04 | 2 | STATE-06 | T-02-06 | `group_id=None` + `assign`/`seek`; replay provably cannot move the `state-machine` group's offsets | integration | `uv run pytest tests/integration/test_replay_offset_range.py -q -p no:cacheprovider` | ❌ W0 (task creates it) | ⬜ pending |
...
|-----|-----------------|-------------|
| STATE-01 | `tests/unit/test_redis_keys_phase2.py`, `tests/integration/test_redis_state_store.py` | 02-02-02, 02-03-03 |
| STATE-02 | `tests/unit/test_engine_transitions.py`, `test_coverage_bounding.py`, `test_engine_tristate_unknown.py`, `test_parsers_opentable.py` | 02-01-02, 02-01-03 |
| STATE-03 | `tests/integration/test_expedite_lua.py`, `test_poller_expedite_release.py`, `tests/unit/test_no_inline_sleep.py` | 02-02-01, 02-03-03 |
| STATE-04 | `tests/unit/test_event_id_determinism.py`, `test_emission_idempotency.py`, `test_no_setnx_expire_pairs.py`, `tests/integration/test_state_machine_chaos.py` | 02-01-03, 02-02-02, 02-03-02, 02-03-03 |
| STATE-05 | `tests/integration/test_migration_0008.py`, `test_availability_events_persistence.py`, `tests/unit/test_service_time_math.py` | 02-02-03, 02-03-02 |
| STATE-06 | `tests/unit/test_replay_determinism.py`, `test_replay_zero_false_events.py`, `test_engine_purity.py`, `tests/integration/test_replay_offset_range.py` | 02-01-03, 02-04-01, 02-04-02, 02-04-03 |
...
- [ ] `tests/unit/factories.py` — `make_raw` / `make_slot` / `make_parsed` builders that REQUIRE an
...
- [ ] `tests/integration/conftest.py` — `reset_shared_db_singletons`, `apply_migrations`,
...
- [ ] `tests/fixtures/raw_streams/` — the directory does not exist today; created by **02-04 Task 1**.
